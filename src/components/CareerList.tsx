...
export function CompareButton({ item, selected, full, onToggle }: { item: Occupation; selected: boolean; full: boolean; onToggle: (id: string) => void }) {
  return <button className={`compare-add ${selected ? 'is-selected' : ''}`} type="button" onClick={() => onToggle(item.noc_code)} disabled={!selected && full} aria-pressed={selected} aria-label={`${selected ? 'Remove' : 'Add'} ${item.title} ${selected ? 'from' : 'to'} comparison`} title={!selected && full ? 'Three careers selected. Remove one to add another.' : selected ? 'Remove from comparison' : 'Add to comparison'}>{selected ? <Check size={15} strokeWidth={2.4} /> : <Plus size={15} strokeWidth={2.2} />}<span>{selected ? 'Added' : 'Compare'}</span></button>
}
...
export function CareerList({ data, sort, page, compared, onSort, onPage, onSelect, onCompare }: Props) {
  ...
  return <section className="career-list research-list" aria-label="Occupation list" id="career-list">
    ...
    <ol className="occupation-directory" start={(actualPage - 1) * pageSize + 1}>{shown.map(({ item, rank }) => <li className="occupation-row" key={item.noc_code} data-noc={item.noc_code}>
      <div className="occupation-identity">{sort !== 'name' && <span className="occupation-rank" aria-label={`Rank ${rank ?? 'unavailable'}`}>{rank == null ? 'N/A' : String(rank).padStart(2, '0')}</span>}<div><button className="career-title-button" type="button" onClick={event => { event.currentTarget.focus({ preventScroll: true }); onSelect(item.noc_code) }}><span>{item.title}</span><ArrowUpRight size={16} aria-hidden="true" /></button><p className="occupation-metadata"><span className="meta-chip meta-noc">NOC {item.noc_code}</span><span className="meta-chip">TEER {teerOf(item)?.id ?? '?'} · {teerOf(item)?.short ?? 'Not available'}</span><span className="meta-outlook"><i style={{ background: outlookOf(item).color }} aria-hidden="true" />{outlookOf(item).label}</span></p></div></div>
      <div className="occupation-pay"><span>Annualized median</span><strong>{annualPay(item.pay)}</strong><small>CAD / year</small></div>
      <div className="occupation-exposure"><span>Relative AI exposure</span><strong>{item.exposure?.toFixed(1) ?? 'N/A'}<small> / 10</small></strong><div className="index-track" aria-hidden="true">{item.exposure != null && <i style={{ left: `${item.exposure * 10}%` }} />}</div></div>
      <CompareButton item={item} selected={compared.includes(item.noc_code)} full={compared.length >= 3} onToggle={onCompare} />
    </li>)}</ol>
    <div className="list-pagination"><p>{ranked.length ? (actualPage - 1) * pageSize + 1 : 0} to {Math.min(actualPage * pageSize, ranked.length)} of {ranked.length} matching occupations</p><div><button type="button" disabled={actualPage <= 1} onClick={() => onPage(actualPage - 1)} aria-label="Previous results page"><ChevronLeft size={17} /></button><span>Page <strong>{actualPage}</strong> of {pages}</span><button type="button" disabled={actualPage >= pages} onClick={() => onPage(actualPage + 1)} aria-label="Next results page"><ChevronRight size={17} /></button></div></div>
