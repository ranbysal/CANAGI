...
import { LAYER_LABELS, MAP_PALETTE, OUTLOOK_COLORS, TEER_COLORS } from '../lib/format'
  ...
  return (
    <div className="layer-bar">
      ...
      {layer === 'education' ? <div className="teer-legend" aria-label="TEER category colours">{TEER_COLORS.map((color, index) => <span key={color}><i style={{ background: color }} />{index}</span>)}</div> : <div className={`color-legend ${layer}`} aria-label={`${legend[0]} to ${legend[1]}`}>
        ...
        <span className="palette-legend" aria-hidden="true">{layer === 'outlook' ? OUTLOOK_COLORS.map((color, index) => <i key={color} style={{ background: color }} title={['Strong surplus', 'Moderate surplus', 'Balance', 'Moderate shortage', 'Strong shortage'][index]} />) : <i style={{ background: `linear-gradient(to right, ${MAP_PALETTE.join(', ')})` }} />}</span>
