...
export function FieldComparison({ data, selected, onChange, onExplore, onMethodology }: { data: Occupation[]; selected: string[]; onChange: (ids: string[]) => void; onExplore: (id: string) => void; onMethodology: () => void }) {
  ...
  return <div className="field-comparison research-comparison">
    <header className="research-heading"><div className="research-heading-copy"><p className="research-eyebrow">THE BROADER PICTURE</p><h2 id="field-comparison-title" tabIndex={-1}><RevealText text="Compare career fields." delay={60} step={70} /></h2><p className="research-deck">Different scales. Different pathways. A shared basis for comparison.</p><p className="research-meta"><span>Canada / NOC 2021</span><span>Full occupation families</span><span>Your career filters stay saved</span></p><button className="text-button research-sources" type="button" onClick={onMethodology}>Sources & methodology <ArrowRight size={14} /></button></div></header>
    <div className="field-selection-toolbar"><p>Choose two or three fields<span>Occupation families, not economic industries</span></p><span className="field-selection-status" role="status">{selected.length} / 3 selected</span></div>
    <div className="field-chips" role="group" aria-label="Career fields to compare">{FIELD_DEFINITIONS.map(([id, name]) => {
      const index = selected.indexOf(id)
      return <button type="button" key={id} aria-pressed={index >= 0} disabled={index < 0 && selected.length >= 3} title={index < 0 && selected.length >= 3 ? 'Three fields selected. Remove one to add another.' : undefined} onClick={() => toggle(id)} style={index >= 0 ? { '--series': `var(--series-${index})` } as CSSProperties : undefined}>{index >= 0 ? <b>{letter(index)}</b> : <Plus size={14} aria-hidden="true" />}{name}</button>
    })}</div>
    <div className="selected-field-index" style={{ '--field-count': Math.max(1, fields.length) } as CSSProperties}>{fields.map((f, index) => <div key={f.id} style={{ '--series': `var(--series-${index})` } as CSSProperties}><div className="field-index-top"><span className="field-letter">{letter(index)}</span><span>NOC {f.id} · {f.count} occupations</span><button type="button" aria-label={`Remove ${f.name} from field comparison`} onClick={() => toggle(f.id)}><X size={16} /></button></div><h3>{f.name}</h3><button className="field-drill" type="button" onClick={() => onExplore(f.id)}>Explore occupations <ArrowRight size={15} /></button></div>)}</div>
